...
func TestLinuxTrayMenuProviderCanReenterHandle(t *testing.T) {
...
func TestLinuxVisibleTrayUnstableProviderFallsBackWithoutRecursion(t *testing.T) {
	handle := &linuxTrayHandle{
...
func TestLinuxTrayProviderReplacementUsesLatestMenu(t *testing.T) {
	handle := &linuxTrayHandle{visible: true}
...
func TestLinuxTrayConcurrentRefreshQueuesWithoutApplyingFallback(t *testing.T) {
	commandDir := t.TempDir()
	commandPath := filepath.Join(commandDir, linuxTrayProviderCommand)
...
func TestLinuxTrayDispatchUsesActiveMenuWhenProviderDoesNotStabilize(t *testing.T) {
	activeClicks := 0
	transientClicks := 0
	providerCalls := 0
	handle := &linuxTrayHandle{
...
func TestLinuxTrayScheduledRefreshPreservesFollowup(t *testing.T) {
	handle := &linuxTrayHandle{
