...
func TestDocsBrowserDemosUseRepresentativeReactAPIs(t *testing.T) {
	required := map[string]string{
		...
		"ChipElementWithSlots(":           "ChipElementWithSlots demo",
...
func TestRightPanelContentBuilds(t *testing.T) {
	...
	content := docsRightPanelContent(doc, th, open, status, status, func(*widgetDoc) ui.Element {
	...
	empty := docsRightPanelContent(nil, th, open, status, status, nil)
	if len(empty) == 0 {
