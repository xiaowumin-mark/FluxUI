...
func App(ctx *ui.Context) ui.Element {
	th := ui.UseTheme(ctx)
	owner := ui.UseState(ctx, "xiaowumin-mark")
	repo := ui.UseState(ctx, "FluxUI")
	...
	return ui.ContainerDecorationElement(
		...
		ui.ScrollViewElement(
			ui.ColumnElement(
				ui.TextElement("网络请求示例（异步，不阻塞 UI）", ui.TextSize(22)),
				ui.SpacerElement(0, 8),
				ui.TextElement("输入 GitHub 仓库 owner/repo，点击「请求仓库信息」后在后台加载。",
					ui.TextSize(13), ui.TextColor(ui.NRGBA(71, 85, 105, 255))),
				ui.SpacerElement(0, 14),
				ui.TextElement("Owner", ui.TextSize(13)),
				ui.TextFieldElement(owner.Value(),
					ui.InputPlaceholder("例如：xiaowumin-mark"),
					ui.InputOnChange(func(ctx *ui.Context, v string) { owner.Set(v) }),
				),
				ui.SpacerElement(0, 10),
				ui.TextElement("Repo", ui.TextSize(13)),
				ui.TextFieldElement(repo.Value(),
				...
				ui.RowElement(
					ui.ButtonElement(
						ui.TextElement("请求仓库信息"),
						ui.Disabled(fetch.Loading()),
						ui.OnClick(func(ctx *ui.Context) { startFetch() }),
					),
					ui.SpacerElement(10, 0),
					ui.PaddingElement(ui.Insets{Top: 8}, ui.TextElement("状态: "+statusText, ui.TextSize(13), ui.TextColor(statusColor))),
				),
				ui.SpacerElement(0, 12),
				ui.ContainerDecorationElement(
					ui.Bg(ui.NRGBA(248, 250, 252, 255)).WithPad(ui.All(12)).WithRad(10),
					ui.ColumnElement(
						ui.TextElement("结果", ui.TextSize(16)),
						ui.SpacerElement(0, 8),
						ui.TextElement("仓库: "+repoField(result, func(r *githubRepo) string { return r.FullName }), ui.TextSize(13)),
						ui.SpacerElement(0, 4),
						ui.TextElement("描述: "+repoField(result, func(r *githubRepo) string { return r.Description }), ui.TextSize(13)),
						ui.SpacerElement(0, 4),
						ui.TextElement("语言: "+repoField(result, func(r *githubRepo) string { return r.Language }), ui.TextSize(13)),
						ui.SpacerElement(0, 4),
						ui.TextElement(repoStats(result), ui.TextSize(13)),
						func() ui.Element {
							if fetch.Error() == nil {
								return nil
							}
							return ui.PaddingElement(
								ui.Insets{Top: 8},
								ui.TextElement("错误: "+fetch.Error().Error(), ui.TextSize(12), ui.TextColor(ui.NRGBA(220, 38, 38, 255))),
							)
						}(),
					),
				),
...
func main() {
	_ = ui.RunElement(App, ui.Title("FluxUI Network Request"), ui.Size(760, 520))
}
